...
:samp:`{Description}:`
  ...
  ``CAF_REGTYPE_LOCK_ALLOC`` and ``CAF_REGTYPE_CRITICAL`` it is the array
  size or one for a scalar.

  When ``CAF_REGTYPE_COARRAY_ALLOC_REGISTER_ONLY`` is used, then only a token
  for an allocatable or pointer component is created.  The ``SIZE`` parameter
  is not used then.  On the contrary when
  ``CAF_REGTYPE_COARRAY_ALLOC_ALLOCATE_ONLY`` is specified, then the
  :samp:`{token}` needs to be registered by a previous call with regtype
  ``CAF_REGTYPE_COARRAY_ALLOC_REGISTER_ONLY`` and either the memory specified
  in the :samp:`{DESC}` 's data-ptr is registered or allocate when the data-ptr is
  ``NULL``.
...
:samp:`{Arguments}:`
  ...
  :samp:`{stat}`        intent(out) For allocatable coarrays, stores the STAT=;
                        may be ``NULL``
  :samp:`{errmsg}`      intent(out) When an error occurs, this will be set to
                        an error message; may be ``NULL``
  :samp:`{errmsg_len}`  the buffer size of errmsg.
  ====================  ==================================================================
...
:samp:`{Arguments}:`
  ...
  :samp:`{stat}`        intent(out) Stores the STAT=; may be NULL
  :samp:`{errmsg}`      intent(out) When an error occurs, this will be set
...
:samp:`{Arguments}:`
  ...
  :samp:`{image_index}`      intent(in)  The ID of the remote image; must be a
                             positive number.
  :samp:`{dest}`             intent(in)  Array descriptor for the remote image for the
                             bounds and the size.  The ``base_addr`` shall not be accessed.
  :samp:`{dst_vector}`       intent(in)  If not NULL, it contains the vector
  ...
  :samp:`{dst_kind}`         intent(in)  Kind of the destination argument
  :samp:`{src_kind}`         intent(in)  Kind of the source argument
  :samp:`{may_require_tmp}`  intent(in)  The variable is ``false`` when
...
:samp:`{Arguments}:`
  ...
  :samp:`{image_index}`      intent(in)  The ID of the remote image; must be a
                             positive number.
  :samp:`{dest}`             intent(out) Array descriptor of the local array to store
                             the data retrieved from the remote image
  :samp:`{src}`              intent(in) Array descriptor for the remote image for the
                             bounds and the size.  The ``base_addr`` shall not be accessed.
  :samp:`{src_vector}`       intent(in)  If not NULL, it contains the vector
...
:samp:`{Arguments}:`
  ...
  :samp:`{dest}`             intent(in) Array descriptor for the destination
                             remote image for the bounds and the size.  The ``base_addr`` shall not be
                             accessed.
  :samp:`{dst_vector}`       intent(int)  If not NULL, it contains the vector
                             subscript of the destination array; the values are relative to the dimension
                             triplet of the :samp:`{dest}` argument.
  :samp:`{src_token}`        intent(in)  An opaque pointer identifying the source
                             coarray.
  :samp:`{src_offset}`       intent(in)  By which amount of bytes the actual data
                             is shifted compared to the base address of the source coarray.
  :samp:`{src_image_index}`  intent(in)  The ID of the source remote image;
                             must be a positive number.
  :samp:`{src}`              intent(in) Array descriptor of the local array to be
                             transferred to the remote image.
  :samp:`{src_vector}`       intent(in) Array descriptor of the local array to
...
:samp:`{Arguments}:`
  ...
  :samp:`{src}`                intent(in) Array descriptor of the local array to be
                               transferred to the remote image
  :samp:`{refs}`               intent(in) The references on the remote array to store
...
:samp:`{Arguments}:`
  ...
  :samp:`{refs}`               intent(in) The references to apply to the remote structure
                               to get the data.
  :samp:`{dst}`                intent(in) Array descriptor of the local array to store
...
:samp:`{Arguments}:`
  ...
  :samp:`{src_image_index}`  intent(in)  The ID of the source remote image;
                             must be a positive number.
  :samp:`{src_refs}`         intent(in) The references to apply to the remote
...
:samp:`{Arguments}:`
  ...
  :samp:`{image_index}`    intent(in)  The ID of the remote image; must be a
                           positive number.
  :samp:`{acquired_lock}`  intent(out) If not NULL, it returns whether lock
                           could be obtained.
  :samp:`{stat}`           intent(out) Stores the STAT=; may be NULL.
  :samp:`{errmsg}`         intent(out) When an error occurs, this will be set to
                           an error message; may be NULL.
...
:samp:`{Arguments}:`
  ...
  :samp:`{image_index}`  intent(in)  The ID of the remote image; must be a
                         positive number; zero indicates the current image when accessed noncoindexed.
  :samp:`{count}`        intent(out)  The number of events currently posted to
                         the event variable.
  :samp:`{stat}`         intent(out)  Stores the STAT=; may be NULL.
  =====================  =============================================================================
...
:samp:`{Arguments}:`
  ...
  :samp:`{value}`        intent(in)  the value to be assigned, passed by reference
  :samp:`{stat}`         intent(out)  Stores the status STAT= and may be NULL.
...
:samp:`{Arguments}:`
  ...
  :samp:`{image_index}`  intent(in)  The ID of the remote image; must be a
                         positive number; zero indicates the current image when used noncoindexed.
  :samp:`{value}`        intent(out)  The variable assigned the atomically
                         referenced variable.
  :samp:`{stat}`         intent(out) Stores the status STAT= and may be NULL.
...
:samp:`{Arguments}:`
  ...
  :samp:`{image_index}`  intent(in)  The ID of the remote image; must be a
                         positive number; zero indicates the current image when used noncoindexed.
  :samp:`{old}`          intent(out)  The value which the atomic variable had
                         just before the cas operation.
  :samp:`{compare}`      intent(in)  The value used for comparision.
  :samp:`{new_val}`      intent(in)  The new value for the atomic variable,
...
:samp:`{Arguments}:`
  ...
  :samp:`{val}`          intent(in)  The new value for the atomic variable,
                         assigned to the atomic variable, if ``compare`` equals the value of the
                         atomic variable.
  :samp:`{stat}`         intent(out)  Stores the status STAT= and may be NULL.
  :samp:`{type}`         intent(in)  the data type, i.e. ``BT_INTEGER`` (1) or
...
:samp:`{Arguments}:`
  ...
  :samp:`{errmsg}`        intent(out)  When an error occurs, this will be set to
                          an error message; may be NULL.
  :samp:`{a_len}`         intent(in)  the string length of argument :samp:`{a}`
  :samp:`{errmsg_len}`    intent(in)  the buffer size of errmsg
  ======================  ==============================================================================
...
:samp:`{Description}:`
  ...
  The :samp:`{opr_flags}` denote the following; the values are bitwise ored.
  ``GFC_CAF_BYREF`` (1) if the result should be returned
  by reference; ``GFC_CAF_HIDDENLEN`` (2) whether the result and argument
  string lengths shall be specified as hidden arguments;
  ``GFC_CAF_ARG_VALUE`` (4) whether the arguments shall be passed by value,
  ``GFC_CAF_ARG_DESC`` (8) whether the arguments shall be passed by descriptor.
