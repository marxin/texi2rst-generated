...
.. gcc-attr:: interrupt
  ...
  re-enable interrupts.  See also the :gcc-attr:`signal` function attribute
  that does not insert a ``SEI`` instruction.  If both :gcc-attr:`signal` and
...
.. gcc-attr:: OS_main, OS_task
  ...
  The :gcc-attr:`OS_main` attribute can be used when there *is
  ...
  The differences to the :gcc-attr:`naked` function attribute are:

  * :gcc-attr:`naked` functions do not have a return instruction whereas 
    :gcc-attr:`OS_main` and ``OS_task`` functions have a ``RET`` or
    ``RETI`` return instruction.

  * :gcc-attr:`naked` functions do not set up a frame for local variables
...
.. gcc-attr:: signal
  ...
  See also the :gcc-attr:`interrupt` function attribute. 

  The AVR hardware globally disables interrupts when an interrupt is executed.
  Interrupt handler functions defined with the :gcc-attr:`signal` attribute
  do not re-enable interrupts.  It is save to enable interrupts in a
  :gcc-attr:`signal` handler.  This 'save' only applies to the code
