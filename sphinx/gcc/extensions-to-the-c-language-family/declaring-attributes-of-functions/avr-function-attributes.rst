...
.. option:: interrupt
  ...
  re-enable interrupts.  See also the :option:`signal` function attribute
  that does not insert a ``SEI`` instruction.  If both :option:`signal` and
  :option:`interrupt` are specified for the same function, :option:`signal`
  is silently ignored.
...
.. option:: no_gccisr
  ...
  the :option:`interrupt` or :option:`signal` attribute aka. interrupt
  service routine (ISR).
...
.. option:: OS_main
  ...
  On AVR, functions with the :option:`OS_main` or ``OS_task`` attribute
  do not save/restore any call-saved register in their prologue/epilogue.

  The :option:`OS_main` attribute can be used when there *is
  ...
  The differences to the :option:`naked` function attribute are:

  * :option:`naked` functions do not have a return instruction whereas 
    :option:`OS_main` and ``OS_task`` functions have a ``RET`` or
    ``RETI`` return instruction.

  * :option:`naked` functions do not set up a frame for local variables
    or a frame pointer whereas :option:`OS_main` and ``OS_task`` do this
    as needed.
...
.. option:: signal
  ...
  See also the :option:`interrupt` function attribute. 

  The AVR hardware globally disables interrupts when an interrupt is executed.
  Interrupt handler functions defined with the :option:`signal` attribute
  do not re-enable interrupts.  It is save to enable interrupts in a
  :option:`signal` handler.  This 'save' only applies to the code
  generated by the compiler and not to the IRQ layout of the
  application which is responsibility of the application.

  If both :option:`signal` and :option:`interrupt` are specified for the same
  function, :option:`signal` is silently ignored.
