...
(see :ref:`jump-patterns`), where all instructions can have an implicit
...
.. toctree::
  ...
  Old style representation of condition codes. <cc0-condition-codes>
  Modern representation of condition codes. <mode_cc-condition-codes>

.. _cc0-condition-codes:
...
.. index:: CC_STATUS_MDEP

MacroCC_STATUS_MDEPC code for a data type which is used for declaring the ``mdep``
component of ``cc_status``.  It defaults to ``int``.

This macro is not used on machines that do not use ``cc0``.

.. index:: CC_STATUS_MDEP_INIT

MacroCC_STATUS_MDEP_INITA C expression to initialize the ``mdep`` field to 'empty'.
...
.. index:: NOTICE_UPDATE_CC

MacroNOTICE_UPDATE_CC(:samp:`{exp}`,:samp:`{insn}`)A C compound statement to set the components of ``cc_status``
appropriately for an insn :samp:`{insn}` whose body is :samp:`{exp}`.  It is
...
:samp:`a4@(102)`.  Therefore, ``NOTICE_UPDATE_CC`` must alter
...
that looks at an attribute (see :ref:`insn-attributes`) named, for example,
