...
.. index:: TARGET_ASM_FILE_START_APP_OFF

Target HookboolTARGET_ASM_FILE_START_APP_OFFIf this flag is true, the text of the macro ``ASM_APP_OFF`` will be
...
.. index:: TARGET_ASM_FILE_START_FILE_DIRECTIVE

Target HookboolTARGET_ASM_FILE_START_FILE_DIRECTIVEIf this flag is true, ``output_file_directive`` will be called
...
.. index:: file_end_indicate_exec_stack

Functionvoidfile_end_indicate_exec_stack()Some systems use a common convention, the :samp:`.note.GNU-stack`
...
.. index:: ASM_COMMENT_START

MacroASM_COMMENT_STARTA C string constant describing how to begin a comment in the target
assembler language.  The compiler assumes that the comment will end at
the end of the line.

.. index:: ASM_APP_ON

MacroASM_APP_ONA C string constant for text to be output before each ``asm``
...
MacroASM_APP_OFFA C string constant for text to be output after each ``asm``
statement or group of consecutive ones.  Normally this is
``"#NO_APP"``, which tells the GNU assembler to resume making the
time-saving assumptions that are valid for ordinary compiler output.

.. index:: ASM_OUTPUT_SOURCE_FILENAME

MacroASM_OUTPUT_SOURCE_FILENAME(:samp:`{stream}`,:samp:`{name}`)A C statement to output COFF information or DWARF debugging information
...
MacroOUTPUT_QUOTED_STRING(:samp:`{stream}`,:samp:`{string}`)A C statement to output the string :samp:`{string}` to the stdio stream
...
.. function:: bool TARGET_ASM_ELF_FLAGS_NUMERIC(unsigned intflags,unsigned int* num)
...
.. function:: section * TARGET_ASM_FUNCTION_SECTION(tree decl,enum node_frequencyfreq,bool startup,bool exit)
...
.. index:: TARGET_HAVE_NAMED_SECTIONS

Common Target HookboolTARGET_HAVE_NAMED_SECTIONSThis flag is true if the target supports ``TARGET_ASM_NAMED_SECTION``.
It must not be modified by command-line option processing.

.. _target_have_switchable_bss_sections:

.. index:: TARGET_HAVE_SWITCHABLE_BSS_SECTIONS

Target HookboolTARGET_HAVE_SWITCHABLE_BSS_SECTIONSThis flag is true if we can create zeroed data by switching to a BSS
...
.. index:: TARGET_ASM_RECORD_GCC_SWITCHES_SECTION

Target Hookconst char *TARGET_ASM_RECORD_GCC_SWITCHES_SECTIONThis is the name of the section that will be created by the example
...
.. index:: TARGET_ASM_BYTE_OP

Target Hookconst char *TARGET_ASM_BYTE_OP
.. index:: TARGET_ASM_ALIGNED_HI_OP

Target Hookconst char *TARGET_ASM_ALIGNED_HI_OP
.. index:: TARGET_ASM_ALIGNED_PSI_OP

Target Hookconst char *TARGET_ASM_ALIGNED_PSI_OP
.. index:: TARGET_ASM_ALIGNED_SI_OP

Target Hookconst char *TARGET_ASM_ALIGNED_SI_OP
.. index:: TARGET_ASM_ALIGNED_PDI_OP

Target Hookconst char *TARGET_ASM_ALIGNED_PDI_OP
.. index:: TARGET_ASM_ALIGNED_DI_OP

Target Hookconst char *TARGET_ASM_ALIGNED_DI_OP
.. index:: TARGET_ASM_ALIGNED_PTI_OP

Target Hookconst char *TARGET_ASM_ALIGNED_PTI_OP
.. index:: TARGET_ASM_ALIGNED_TI_OP

Target Hookconst char *TARGET_ASM_ALIGNED_TI_OP
.. index:: TARGET_ASM_UNALIGNED_HI_OP

Target Hookconst char *TARGET_ASM_UNALIGNED_HI_OP
.. index:: TARGET_ASM_UNALIGNED_PSI_OP

Target Hookconst char *TARGET_ASM_UNALIGNED_PSI_OP
.. index:: TARGET_ASM_UNALIGNED_SI_OP

Target Hookconst char *TARGET_ASM_UNALIGNED_SI_OP
.. index:: TARGET_ASM_UNALIGNED_PDI_OP

Target Hookconst char *TARGET_ASM_UNALIGNED_PDI_OP
.. index:: TARGET_ASM_UNALIGNED_DI_OP

Target Hookconst char *TARGET_ASM_UNALIGNED_DI_OP
.. index:: TARGET_ASM_UNALIGNED_PTI_OP

Target Hookconst char *TARGET_ASM_UNALIGNED_PTI_OP
.. index:: TARGET_ASM_UNALIGNED_TI_OP

Target Hookconst char *TARGET_ASM_UNALIGNED_TI_OPThese hooks specify assembly directives for creating certain kinds
...
.. function:: bool TARGET_ASM_INTEGER(rtx x,unsigned intsize,int aligned_p)
...
.. index:: ASM_OUTPUT_ASCII

MacroASM_OUTPUT_ASCII(:samp:`{stream}`,:samp:`{ptr}`,:samp:`{len}`)A C statement to output to the stdio stream :samp:`{stream}` an assembler
...
.. index:: ASM_OUTPUT_FDESC

MacroASM_OUTPUT_FDESC(:samp:`{stream}`,:samp:`{decl}`,:samp:`{n}`)A C statement to output word :samp:`{n}` of a function descriptor for
:samp:`{decl}`.  This must be defined if ``TARGET_VTABLE_USES_DESCRIPTORS``
is defined, and is otherwise unused.

.. index:: CONSTANT_POOL_BEFORE_FUNCTION

MacroCONSTANT_POOL_BEFORE_FUNCTIONYou may define this macro as a C expression.  You should define the
...
.. index:: ASM_OUTPUT_POOL_PROLOGUE

MacroASM_OUTPUT_POOL_PROLOGUE(:samp:`{file}`,:samp:`{funname}`,:samp:`{fundecl}`,:samp:`{size}`)A C statement to output assembler commands to define the start of the
...
.. index:: ASM_OUTPUT_SPECIAL_POOL_ENTRY

MacroASM_OUTPUT_SPECIAL_POOL_ENTRY(:samp:`{file}`,:samp:`{x}`,:samp:`{mode}`,:samp:`{align}`,:samp:`{labelno}`,:samp:`{jumpto}`)A C statement (with or without semicolon) to output a constant in the
...
.. index:: ASM_OUTPUT_POOL_EPILOGUE

MacroASM_OUTPUT_POOL_EPILOGUE(:samp:`{file}`:samp:`{funname}`:samp:`{fundecl}`:samp:`{size}`)A C statement to output assembler commands to at the end of the constant
...
.. index:: IS_ASM_LOGICAL_LINE_SEPARATOR

MacroIS_ASM_LOGICAL_LINE_SEPARATOR(:samp:`{C}`,:samp:`{STR}`)Define this macro as a C expression which is nonzero if :samp:`{C}` is
...
.. index:: TARGET_ASM_OPEN_PAREN

Target Hookconst char *TARGET_ASM_OPEN_PAREN
.. index:: TARGET_ASM_CLOSE_PAREN

Target Hookconst char *TARGET_ASM_CLOSE_PARENThese target hooks are C string constants, describing the syntax in the
...
.. index:: REAL_VALUE_TO_TARGET_SINGLE

MacroREAL_VALUE_TO_TARGET_SINGLE(:samp:`{x}`,:samp:`{l}`)
.. index:: REAL_VALUE_TO_TARGET_DOUBLE

MacroREAL_VALUE_TO_TARGET_DOUBLE(:samp:`{x}`,:samp:`{l}`)
.. index:: REAL_VALUE_TO_TARGET_LONG_DOUBLE

MacroREAL_VALUE_TO_TARGET_LONG_DOUBLE(:samp:`{x}`,:samp:`{l}`)
.. index:: REAL_VALUE_TO_TARGET_DECIMAL32

MacroREAL_VALUE_TO_TARGET_DECIMAL32(:samp:`{x}`,:samp:`{l}`)
.. index:: REAL_VALUE_TO_TARGET_DECIMAL64
...
.. index:: ASM_OUTPUT_COMMON

MacroASM_OUTPUT_COMMON(:samp:`{stream}`,:samp:`{name}`,:samp:`{size}`,:samp:`{rounded}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_OUTPUT_ALIGNED_COMMON

MacroASM_OUTPUT_ALIGNED_COMMON(:samp:`{stream}`,:samp:`{name}`,:samp:`{size}`,:samp:`{alignment}`)Like ``ASM_OUTPUT_COMMON`` except takes the required alignment as a
separate, explicit argument.  If you define this macro, it is used in
place of ``ASM_OUTPUT_COMMON``, and gives you more flexibility in
handling the required alignment of the variable.  The alignment is specified
as the number of bits.

.. index:: ASM_OUTPUT_ALIGNED_DECL_COMMON

MacroASM_OUTPUT_ALIGNED_DECL_COMMON(:samp:`{stream}`,:samp:`{decl}`,:samp:`{name}`,:samp:`{size}`,:samp:`{alignment}`)Like ``ASM_OUTPUT_ALIGNED_COMMON`` except that :samp:`{decl}` of the
...
.. index:: ASM_OUTPUT_ALIGNED_BSS

MacroASM_OUTPUT_ALIGNED_BSS(:samp:`{stream}`,:samp:`{decl}`,:samp:`{name}`,:samp:`{size}`,:samp:`{alignment}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_OUTPUT_LOCAL

MacroASM_OUTPUT_LOCAL(:samp:`{stream}`,:samp:`{name}`,:samp:`{size}`,:samp:`{rounded}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_OUTPUT_ALIGNED_LOCAL

MacroASM_OUTPUT_ALIGNED_LOCAL(:samp:`{stream}`,:samp:`{name}`,:samp:`{size}`,:samp:`{alignment}`)Like ``ASM_OUTPUT_LOCAL`` except takes the required alignment as a
separate, explicit argument.  If you define this macro, it is used in
place of ``ASM_OUTPUT_LOCAL``, and gives you more flexibility in
handling the required alignment of the variable.  The alignment is specified
as the number of bits.

.. index:: ASM_OUTPUT_ALIGNED_DECL_LOCAL

MacroASM_OUTPUT_ALIGNED_DECL_LOCAL(:samp:`{stream}`,:samp:`{decl}`,:samp:`{name}`,:samp:`{size}`,:samp:`{alignment}`)Like ``ASM_OUTPUT_ALIGNED_LOCAL`` except that :samp:`{decl}` of the
variable to be output, if there is one, or ``NULL_TREE`` if there
is no corresponding variable.  If you define this macro, GCC will use it
...
.. index:: ASM_OUTPUT_FUNCTION_LABEL
...
.. index:: ASM_OUTPUT_INTERNAL_LABEL

MacroASM_OUTPUT_INTERNAL_LABEL(:samp:`{stream}`,:samp:`{name}`)Identical to ``ASM_OUTPUT_LABEL``, except that :samp:`{name}` is known
...
MacroSIZE_ASM_OPA C string containing the appropriate assembler directive to specify the
...
.. index:: ASM_OUTPUT_SIZE_DIRECTIVE

MacroASM_OUTPUT_SIZE_DIRECTIVE(:samp:`{stream}`,:samp:`{name}`,:samp:`{size}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_OUTPUT_MEASURED_SIZE

MacroASM_OUTPUT_MEASURED_SIZE(:samp:`{stream}`,:samp:`{name}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: NO_DOLLAR_IN_LABEL

MacroNO_DOLLAR_IN_LABELDefine this macro if the assembler does not accept the character
:samp:`$` in label names.  By default constructors and destructors in
G++ have :samp:`$` in the identifiers.  If this macro is defined,
:samp:`.` is used instead.

.. index:: NO_DOT_IN_LABEL

MacroNO_DOT_IN_LABELDefine this macro if the assembler does not accept the character
...
MacroTYPE_ASM_OPA C string containing the appropriate assembler directive to specify the
...
.. index:: TYPE_OPERAND_FMT

MacroTYPE_OPERAND_FMTA C string which specifies (using ``printf`` syntax) the format of
...
.. index:: ASM_OUTPUT_TYPE_DIRECTIVE

MacroASM_OUTPUT_TYPE_DIRECTIVE(:samp:`{stream}`,:samp:`{type}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_DECLARE_FUNCTION_NAME

MacroASM_DECLARE_FUNCTION_NAME(:samp:`{stream}`,:samp:`{name}`,:samp:`{decl}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_DECLARE_FUNCTION_SIZE

MacroASM_DECLARE_FUNCTION_SIZE(:samp:`{stream}`,:samp:`{name}`,:samp:`{decl}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_DECLARE_COLD_FUNCTION_NAME

MacroASM_DECLARE_COLD_FUNCTION_NAME(:samp:`{stream}`,:samp:`{name}`,:samp:`{decl}`)A C statement (sans semicolon) to output to the stdio stream
:samp:`{stream}` any text necessary for declaring the name :samp:`{name}` of a
...
.. index:: ASM_DECLARE_COLD_FUNCTION_SIZE

MacroASM_DECLARE_COLD_FUNCTION_SIZE(:samp:`{stream}`,:samp:`{name}`,:samp:`{decl}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_DECLARE_REGISTER_GLOBAL

MacroASM_DECLARE_REGISTER_GLOBAL(:samp:`{stream}`,:samp:`{decl}`,:samp:`{regno}`,:samp:`{name}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_FINISH_DECLARE_OBJECT

MacroASM_FINISH_DECLARE_OBJECT(:samp:`{stream}`,:samp:`{decl}`,:samp:`{toplevel}`,:samp:`{atend}`)A C statement (sans semicolon) to finish up declaring a variable name
...
.. index:: ASM_WEAKEN_LABEL

MacroASM_WEAKEN_LABEL(:samp:`{stream}`,:samp:`{name}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_WEAKEN_DECL

MacroASM_WEAKEN_DECL(:samp:`{stream}`,:samp:`{decl}`,:samp:`{name}`,:samp:`{value}`)Combines (and replaces) the function of ``ASM_WEAKEN_LABEL`` and
...
.. index:: ASM_OUTPUT_WEAKREF

MacroASM_OUTPUT_WEAKREF(:samp:`{stream}`,:samp:`{decl}`,:samp:`{name}`,:samp:`{value}`)Outputs a directive that enables :samp:`{name}` to be used to refer to
symbol :samp:`{value}` with weak-symbol semantics.  ``decl`` is the
declaration of ``name``.

.. index:: SUPPORTS_WEAK

MacroSUPPORTS_WEAKA preprocessor constant expression which evaluates to true if the target
...
.. index:: TARGET_SUPPORTS_WEAK

MacroTARGET_SUPPORTS_WEAKA C expression which evaluates to true if the target supports weak symbols.
...
.. index:: MAKE_DECL_ONE_ONLY

MacroMAKE_DECL_ONE_ONLY(:samp:`{decl}`)A C statement (sans semicolon) to mark :samp:`{decl}` to be emitted as a
...
MacroSUPPORTS_ONE_ONLYA C expression which evaluates to true if the target supports one-only
semantics.
...
.. index:: TARGET_WEAK_NOT_IN_ARCHIVE_TOC

MacroTARGET_WEAK_NOT_IN_ARCHIVE_TOCA C expression that evaluates to true if the target's linker expects
...
.. index:: ASM_OUTPUT_EXTERNAL

MacroASM_OUTPUT_EXTERNAL(:samp:`{stream}`,:samp:`{decl}`,:samp:`{name}`)A C statement (sans semicolon) to output to the stdio stream
...
.. index:: ASM_OUTPUT_LABELREF

MacroASM_OUTPUT_LABELREF(:samp:`{stream}`,:samp:`{name}`)A C statement (sans semicolon) to output to the stdio stream
...
MacroASM_OUTPUT_SYMBOL_REF(:samp:`{stream}`,:samp:`{sym}`)A C statement (sans semicolon) to output a reference to
...
.. function:: void TARGET_ASM_INTERNAL_LABEL(FILE *stream,const char* prefix,unsigned longlabelno)
...
.. index:: ASM_OUTPUT_DEBUG_LABEL

MacroASM_OUTPUT_DEBUG_LABEL(:samp:`{stream}`,:samp:`{prefix}`,:samp:`{num}`)A C statement to output to the stdio stream :samp:`{stream}` a debug info
...
.. index:: ASM_GENERATE_INTERNAL_LABEL

MacroASM_GENERATE_INTERNAL_LABEL(:samp:`{string}`,:samp:`{prefix}`,:samp:`{num}`)A C statement to store into the string :samp:`{string}` a label whose name
...
.. index:: ASM_OUTPUT_DEF

MacroASM_OUTPUT_DEF(:samp:`{stream}`,:samp:`{name}`,:samp:`{value}`)A C statement to output to the stdio stream :samp:`{stream}` assembler code
...
.. index:: ASM_OUTPUT_DEF_FROM_DECLS

MacroASM_OUTPUT_DEF_FROM_DECLS(:samp:`{stream}`,:samp:`{decl_of_name}`,:samp:`{decl_of_value}`)A C statement to output to the stdio stream :samp:`{stream}` assembler code
...
.. index:: TARGET_DEFERRED_OUTPUT_DEFS

MacroTARGET_DEFERRED_OUTPUT_DEFS(:samp:`{decl_of_name}`,:samp:`{decl_of_value}`)A C statement that evaluates to true if the assembler code which defines
...
.. index:: ASM_OUTPUT_WEAK_ALIAS

MacroASM_OUTPUT_WEAK_ALIAS(:samp:`{stream}`,:samp:`{name}`,:samp:`{value}`)A C statement to output to the stdio stream :samp:`{stream}` assembler code
...
.. index:: OBJC_GEN_METHOD_LABEL

MacroOBJC_GEN_METHOD_LABEL(:samp:`{buf}`,:samp:`{is_inst}`,:samp:`{class_name}`,:samp:`{cat_name}`,:samp:`{sel_name}`)Define this macro to override the default assembler names used for
Objective-C methods.
...
MacroINIT_SECTION_ASM_OPIf defined, a C string constant, including spacing, for the assembler
...
.. index:: HAS_INIT_SECTION

MacroHAS_INIT_SECTIONIf defined, ``main`` will not call ``__main`` as described above.
This macro should be defined for systems that control start-up code
on a symbol-by-symbol basis, such as OSF/1, and should not
be defined explicitly for systems that support ``INIT_SECTION_ASM_OP``.

.. index:: LD_INIT_SWITCH

MacroLD_INIT_SWITCHIf defined, a C string constant for a switch that tells the linker that
the following symbol is an initialization routine.

.. index:: LD_FINI_SWITCH

MacroLD_FINI_SWITCHIf defined, a C string constant for a switch that tells the linker that
the following symbol is a finalization routine.

.. index:: COLLECT_SHARED_INIT_FUNC
...
.. index:: COLLECT_SHARED_FINI_FUNC
...
MacroINVOKE__mainIf defined, ``main`` will call ``__main`` despite the presence of
``INIT_SECTION_ASM_OP``.  This macro should be defined for systems
where the init section is not actually run automatically, but is still
useful for collecting the lists of constructors and destructors.

.. index:: SUPPORTS_INIT_PRIORITY

MacroSUPPORTS_INIT_PRIORITYIf nonzero, the C++ ``init_priority`` attribute is supported and the
compiler should emit instructions to control the order of initialization
of objects.  If zero, the compiler will issue an error message upon
encountering an ``init_priority`` attribute.

.. index:: TARGET_HAVE_CTORS_DTORS

Target HookboolTARGET_HAVE_CTORS_DTORSThis value is true if the target supports some 'native' method of
...
.. index:: OBJECT_FORMAT_COFF

MacroOBJECT_FORMAT_COFFDefine this macro if the system uses COFF (Common Object File Format)
...
.. index:: REAL_NM_FILE_NAME

MacroREAL_NM_FILE_NAMEDefine this macro as a C string constant containing the file name to use
to execute :command:`nm`.  The default is to search the path normally for
:command:`nm`.

.. index:: NM_FLAGS

MacroNM_FLAGS:command:`collect2` calls :command:`nm` to scan object files for static
...
MacroLDD_SUFFIXDefine this macro to a C string constant containing the name of the program
which lists dynamic dependencies, like :command:`ldd` under SunOS 4.

.. index:: PARSE_LDD_OUTPUT

MacroPARSE_LDD_OUTPUT(:samp:`{ptr}`)Define this macro to be C code that extracts filenames from the output
...
MacroSHLIB_SUFFIXDefine this macro to a C string constant containing the default shared
...
MacroREGISTER_NAMESA C initializer containing the assembler's names for the machine
registers, each one as a C string constant.  This is what translates
register numbers in the compiler into assembler language.

.. index:: ADDITIONAL_REGISTER_NAMES

MacroADDITIONAL_REGISTER_NAMESIf defined, a C initializer for an array of structures containing a name
and a register number.  This macro defines additional names for hard
registers, thus allowing the ``asm`` option in declarations to refer
to registers using alternate names.

.. index:: OVERLAPPING_REGISTER_NAMES

MacroOVERLAPPING_REGISTER_NAMESIf defined, a C initializer for an array of structures containing a
...
.. index:: ASM_OUTPUT_OPCODE

MacroASM_OUTPUT_OPCODE(:samp:`{stream}`,:samp:`{ptr}`)Define this macro if you are using an unusual assembler that
...
MacroPRINT_OPERAND(:samp:`{stream}`,:samp:`{x}`,:samp:`{code}`)A C compound statement to output to stdio stream :samp:`{stream}` the
...
.. index:: PRINT_OPERAND_PUNCT_VALID_P

MacroPRINT_OPERAND_PUNCT_VALID_P(:samp:`{code}`)A C expression which evaluates to true if :samp:`{code}` is a valid
punctuation character for use in the ``PRINT_OPERAND`` macro.  If
``PRINT_OPERAND_PUNCT_VALID_P`` is not defined, it means that no
punctuation characters (except for the standard one, :samp:`%`) are used
in this way.

.. index:: PRINT_OPERAND_ADDRESS

MacroPRINT_OPERAND_ADDRESS(:samp:`{stream}`,:samp:`{x}`)A C compound statement to output to stdio stream :samp:`{stream}` the
...
.. index:: DBR_OUTPUT_SEQEND

MacroDBR_OUTPUT_SEQEND(:samp:`{file}`)A C statement, to be executed after all slot-filler instructions have
...
.. index:: REGISTER_PREFIX

MacroREGISTER_PREFIX
.. index:: LOCAL_LABEL_PREFIX

MacroLOCAL_LABEL_PREFIX
.. index:: USER_LABEL_PREFIX

MacroUSER_LABEL_PREFIX
.. index:: IMMEDIATE_PREFIX

MacroIMMEDIATE_PREFIXIf defined, C string expressions to be used for the :samp:`%R`, :samp:`%L`,
...
.. index:: ASM_FPRINTF_EXTENSIONS

MacroASM_FPRINTF_EXTENSIONS(:samp:`{file}`,:samp:`{argptr}`,:samp:`{format}`)If defined this macro should expand to a series of ``case``
...
.. index:: ASSEMBLER_DIALECT

MacroASSEMBLER_DIALECTIf your target supports multiple dialects of assembler language (such as
...
.. index:: ASM_OUTPUT_REG_PUSH

MacroASM_OUTPUT_REG_PUSH(:samp:`{stream}`,:samp:`{regno}`)A C expression to output to :samp:`{stream}` some assembler code
which will push hard register number :samp:`{regno}` onto the stack.
The code need not be optimal, since this macro is used only when
profiling.

.. index:: ASM_OUTPUT_REG_POP

MacroASM_OUTPUT_REG_POP(:samp:`{stream}`,:samp:`{regno}`)A C expression to output to :samp:`{stream}` some assembler code
...
.. index:: ASM_OUTPUT_ADDR_DIFF_ELT

MacroASM_OUTPUT_ADDR_DIFF_ELT(:samp:`{stream}`,:samp:`{body}`,:samp:`{value}`,:samp:`{rel}`)A C statement to output to the stdio stream :samp:`{stream}` an assembler
...
MacroASM_OUTPUT_ADDR_VEC_ELT(:samp:`{stream}`,:samp:`{value}`)This macro should be provided on machines where the addresses
...
.. index:: ASM_OUTPUT_CASE_END

MacroASM_OUTPUT_CASE_END(:samp:`{stream}`,:samp:`{num}`,:samp:`{table}`)Define this if something special must be output at the end of a
...
.. index:: TARGET_ASM_UNWIND_EMIT_BEFORE_INSN

Target HookboolTARGET_ASM_UNWIND_EMIT_BEFORE_INSNTrue if the ``TARGET_ASM_UNWIND_EMIT`` hook should be called before the assembly for :samp:`{insn}` has been emitted, false if the hook should be called afterward.
...
.. index:: EH_FRAME_SECTION_NAME

MacroEH_FRAME_SECTION_NAMEIf defined, a C string constant for the name of the section containing
...
.. index:: EH_FRAME_THROUGH_COLLECT2

MacroEH_FRAME_THROUGH_COLLECT2If defined, DWARF 2 frame unwind information will identified by
...
.. index:: EH_TABLES_CAN_BE_READ_ONLY

MacroEH_TABLES_CAN_BE_READ_ONLYDefine this macro to 1 if your target is such that no frame unwind
...
.. index:: MASK_RETURN_ADDR

MacroMASK_RETURN_ADDRAn rtx used to mask the return address found via ``RETURN_ADDR_RTX``, so
that it does not contain any extraneous set bits in it.

.. index:: DWARF2_UNWIND_INFO
...
.. index:: TARGET_UNWIND_TABLES_DEFAULT

Common Target HookboolTARGET_UNWIND_TABLES_DEFAULTThis variable should be set to ``true`` if the target ABI requires unwinding
...
.. index:: DWARF_CIE_DATA_ALIGNMENT

MacroDWARF_CIE_DATA_ALIGNMENTThis macro need only be defined if the target might save registers in the
...
.. index:: TARGET_TERMINATE_DW2_EH_FRAME_INFO

Target HookboolTARGET_TERMINATE_DW2_EH_FRAME_INFOContains the value true if the target should add a zero word onto the
...
.. index:: TARGET_ARM_EABI_UNWINDER

Target HookboolTARGET_ARM_EABI_UNWINDERThis flag should be set to ``true`` on targets that use an ARM EABI
...
MacroLOOP_ALIGN(:samp:`{label}`)The alignment (log base 2) to put in front of :samp:`{label}` that heads
...
MacroLABEL_ALIGN(:samp:`{label}`)The alignment (log base 2) to put in front of :samp:`{label}`.
...
.. index:: ASM_OUTPUT_SKIP

MacroASM_OUTPUT_SKIP(:samp:`{stream}`,:samp:`{nbytes}`)A C statement to output to the stdio stream :samp:`{stream}` an assembler
...
.. index:: ASM_NO_SKIP_IN_TEXT

MacroASM_NO_SKIP_IN_TEXTDefine this macro if ``ASM_OUTPUT_SKIP`` should not be used in the
text section because it fails to put zeros in the bytes that are skipped.
This is true on many Unix systems, where the pseudo-op to skip bytes
produces no-op instructions rather than zeros when used in the text
section.

.. index:: ASM_OUTPUT_ALIGN

MacroASM_OUTPUT_ALIGN(:samp:`{stream}`,:samp:`{power}`)A C statement to output to the stdio stream :samp:`{stream}` an assembler
command to advance the location counter to a multiple of 2 to the
:samp:`{power}` bytes.  :samp:`{power}` will be a C expression of type ``int``.

.. index:: ASM_OUTPUT_ALIGN_WITH_NOP

MacroASM_OUTPUT_ALIGN_WITH_NOP(:samp:`{stream}`,:samp:`{power}`)Like ``ASM_OUTPUT_ALIGN``, except that the 'nop' instruction is used
for padding, if necessary.

.. index:: ASM_OUTPUT_MAX_SKIP_ALIGN

MacroASM_OUTPUT_MAX_SKIP_ALIGN(:samp:`{stream}`,:samp:`{power}`,:samp:`{max_skip}`)A C statement to output to the stdio stream :samp:`{stream}` an assembler
command to advance the location counter to a multiple of 2 to the
