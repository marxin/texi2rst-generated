...
.. index:: DBX_REGISTER_NUMBER

MacroDBX_REGISTER_NUMBER(:samp:`{regno}`)A C expression that returns the DBX register number for the compiler
...
.. index:: DEBUGGER_AUTO_OFFSET

MacroDEBUGGER_AUTO_OFFSET(:samp:`{x}`)A C expression that returns the integer offset value for an automatic
...
.. index:: DEBUGGER_ARG_OFFSET

MacroDEBUGGER_ARG_OFFSET(:samp:`{offset}`,:samp:`{x}`)A C expression that returns the integer offset value for an argument
having address :samp:`{x}` (an RTL expression).  The nominal offset is
:samp:`{offset}`.

.. index:: PREFERRED_DEBUGGING_TYPE

MacroPREFERRED_DEBUGGING_TYPEA C expression that returns the type of debugging output GCC should
...
.. index:: DBX_DEBUGGING_INFO

MacroDBX_DEBUGGING_INFODefine this macro if GCC should produce debugging output for DBX
in response to the :option:`-g` option.

.. index:: XCOFF_DEBUGGING_INFO

MacroXCOFF_DEBUGGING_INFODefine this macro if GCC should produce XCOFF format debugging output
in response to the :option:`-g` option.  This is a variant of DBX format.

.. index:: DEFAULT_GDB_EXTENSIONS

MacroDEFAULT_GDB_EXTENSIONSDefine this macro to control whether GCC should by default generate
GDB's extended version of DBX debugging information (assuming DBX-format
debugging information is enabled at all).  If you don't define the
macro, the default is 1: always generate the extended information
if there is any occasion to.

.. index:: DEBUG_SYMS_TEXT

MacroDEBUG_SYMS_TEXTDefine this macro if all ``.stabs`` commands should be output while
in the text section.

.. index:: ASM_STABS_OP

MacroASM_STABS_OPA C string constant, including spacing, naming the assembler pseudo op to
...
MacroASM_STABD_OPA C string constant, including spacing, naming the assembler pseudo op to
...
MacroASM_STABN_OPA C string constant, including spacing, naming the assembler pseudo op to
...
.. index:: DBX_CONTIN_LENGTH

MacroDBX_CONTIN_LENGTHA symbol name in DBX-format debugging information is normally
...
.. index:: DBX_CONTIN_CHAR

MacroDBX_CONTIN_CHARNormally continuation is indicated by adding a :samp:`\` character to
the end of a ``.stabs`` string when a continuation follows.  To use
a different character instead, define this macro as a character
constant for the character you want to use.  Do not define this macro
if backslash is correct for your system.

.. index:: DBX_STATIC_STAB_DATA_SECTION

MacroDBX_STATIC_STAB_DATA_SECTIONDefine this macro if it is necessary to go to the data section before
outputting the :samp:`.stabs` pseudo-op for a non-global static
variable.

.. index:: DBX_TYPE_DECL_STABS_CODE

MacroDBX_TYPE_DECL_STABS_CODEThe value to use in the 'code' field of the ``.stabs`` directive
for a typedef.  The default is ``N_LSYM``.

.. index:: DBX_STATIC_CONST_VAR_CODE

MacroDBX_STATIC_CONST_VAR_CODEThe value to use in the 'code' field of the ``.stabs`` directive
for a static variable located in the text section.  DBX format does not
provide any 'right' way to do this.  The default is ``N_FUN``.

.. index:: DBX_REGPARM_STABS_CODE

MacroDBX_REGPARM_STABS_CODEThe value to use in the 'code' field of the ``.stabs`` directive
for a parameter passed in registers.  DBX format does not provide any
'right' way to do this.  The default is ``N_RSYM``.

.. index:: DBX_REGPARM_STABS_LETTER

MacroDBX_REGPARM_STABS_LETTERThe letter to use in DBX symbol data to identify a symbol as a parameter
passed in registers.  DBX format does not customarily provide any way to
do this.  The default is ``'P'``.

.. index:: DBX_FUNCTION_FIRST

MacroDBX_FUNCTION_FIRSTDefine this macro if the DBX information for a function and its
...
MacroDBX_USE_BINCLDefine this macro if GCC should generate ``N_BINCL`` and
...
.. index:: DBX_OUTPUT_SOURCE_LINE

MacroDBX_OUTPUT_SOURCE_LINE(:samp:`{stream}`,:samp:`{line}`,:samp:`{counter}`)A C statement to output DBX debugging information before code for line
...
.. index:: NO_DBX_FUNCTION_END

MacroNO_DBX_FUNCTION_ENDSome stabs encapsulation formats (in particular ECOFF), cannot handle the
``.stabs "",N_FUN,,0,0,Lscope-function-1`` gdb dbx extension construct.
On those machines, define this macro to turn this feature off without
disturbing the rest of the gdb extensions.

.. index:: NO_DBX_BNSYM_ENSYM

MacroNO_DBX_BNSYM_ENSYMSome assemblers cannot handle the ``.stabd BNSYM/ENSYM,0,0`` gdb dbx
...
.. index:: NO_DBX_MAIN_SOURCE_DIRECTORY

MacroNO_DBX_MAIN_SOURCE_DIRECTORYDefine this macro, with value 1, if GCC should not emit an indication
of the current directory for compilation and current source language at
the beginning of the file.

.. index:: NO_DBX_GCC_MARKER

MacroNO_DBX_GCC_MARKERDefine this macro, with value 1, if GCC should not emit an indication
...
.. index:: DBX_OUTPUT_MAIN_SOURCE_FILE_END

MacroDBX_OUTPUT_MAIN_SOURCE_FILE_END(:samp:`{stream}`,:samp:`{name}`)A C statement to output DBX debugging information at the end of
...
.. index:: DBX_OUTPUT_NULL_N_SO_AT_MAIN_SOURCE_FILE_END

MacroDBX_OUTPUT_NULL_N_SO_AT_MAIN_SOURCE_FILE_ENDDefine this macro *instead of* defining
...
.. index:: DWARF2_DEBUGGING_INFO

MacroDWARF2_DEBUGGING_INFODefine this macro if GCC should produce dwarf version 2 format
...
.. index:: DWARF2_FRAME_INFO

MacroDWARF2_FRAME_INFODefine this macro to a nonzero value if GCC should always output
...
.. index:: DWARF2_ASM_LINE_DEBUG_INFO

MacroDWARF2_ASM_LINE_DEBUG_INFODefine this macro to be a nonzero value if the assembler can generate Dwarf 2
line debug info sections.  This will result in much more compact line number
tables, and hence is desirable if it works.

.. index:: DWARF2_ASM_VIEW_DEBUG_INFO

MacroDWARF2_ASM_VIEW_DEBUG_INFODefine this macro to be a nonzero value if the assembler supports view
...
.. index:: TARGET_WANT_DEBUG_PUB_SECTIONS

Target HookboolTARGET_WANT_DEBUG_PUB_SECTIONSTrue if the ``.debug_pubtypes`` and ``.debug_pubnames`` sections should be emitted.  These sections are not used on most platforms, and in particular GDB does not use them.

.. index:: TARGET_DELAY_SCHED2

Target HookboolTARGET_DELAY_SCHED2True if sched2 is not to be run at its normal place.
This usually means it will be run as part of machine-specific reorg.

.. index:: TARGET_DELAY_VARTRACK

Target HookboolTARGET_DELAY_VARTRACKTrue if vartrack is not to be run at its normal place.
This usually means it will be run as part of machine-specific reorg.

.. index:: TARGET_NO_REGISTER_ALLOCATION

Target HookboolTARGET_NO_REGISTER_ALLOCATIONTrue if register allocation and the passes
...
MacroASM_OUTPUT_DWARF_VMS_DELTA(:samp:`{stream}`,:samp:`{size}`,:samp:`{label1}`,:samp:`{label2}`)A C statement to issue assembly directives that create a difference
...
MacroASM_OUTPUT_DWARF_DATAREL(:samp:`{stream}`,:samp:`{size}`,:samp:`{label}`)A C statement to issue assembly directives that create a reference to the
given :samp:`{label}` relative to the dbase, using an integer of the given :samp:`{size}`.

.. index:: ASM_OUTPUT_DWARF_TABLE_REF

MacroASM_OUTPUT_DWARF_TABLE_REF(:samp:`{label}`)A C statement to issue assembly directives that create a reference to
...
.. index:: VMS_DEBUGGING_INFO

MacroVMS_DEBUGGING_INFODefine this macro if GCC should produce debugging output for VMS
