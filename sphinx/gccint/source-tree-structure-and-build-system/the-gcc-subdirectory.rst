...
:samp:`po`
  ...
  various languages, :samp:`:samp:`{language}`.po`.  This directory also
...
* Each language subdirectory has a file
  :samp:`:samp:`{language}` /config-lang.in` that is used for
...
* :samp:`Makefile` is constructed from :samp:`Makefile.in`, together with
  ...
  Fragments <fragments>`) :samp:`t- :samp:`{target}`` and :samp:`x- :samp:`{host}`` from
  :samp:`config`, if any, and language Makefile fragments
  :samp:`:samp:`{language}` /Make-lang.in`.
...
:samp:`xm- :samp:`{machine}`.h` header, if any, appropriate to the host,
...
* :samp:`tm_p.h`, which includes the header :samp:`:samp:`{machine}` -protos.h`
  that contains prototypes for functions in the target
  :samp:`:samp:`{machine}`.c` file.  The
  :samp:`:samp:`{machine}` -protos.h` header is included after the :samp:`rtl.h`
...
:samp:`stage{N}-start ({N} = 1...4, profile, feedback)`
  ...
  you should be able to work directly in the :samp:`stage :samp:`{N}` -gcc`
...
in :samp:`:samp:`{libsubdir}` /include`.  Headers for non-C runtime
...
:samp:`:samp:`{libsubdir}` /include/syslimits.h`, which is copied from
...
files :samp:`:samp:`{language}` /*.texi`.  Common files
...
that file; a file :samp:`:samp:`{name}`.texi` must only appear once in the
...
:samp:`:samp:`{name}`.texi` should only include other files in its own
...
* A directory :samp:`:samp:`{language}`` under :samp:`gcc` containing source
...
A front end :samp:`:samp:`{language}`` directory contains the source files
...
``outputs``
  ...
  be used to create a file :samp:`:samp:`{language}` /Makefile` from
  :samp:`:samp:`{language}` /Makefile.in`, but this is deprecated, building
...
* A directory :samp:`:samp:`{machine}`` under :samp:`gcc/config`, containing a
  machine description :samp:`:samp:`{machine}`.md` file (see :ref:`Machine Descriptions <machine-desc>`), header files :samp:`:samp:`{machine}`.h` and
  :samp:`:samp:`{machine}` -protos.h` and a source file :samp:`:samp:`{machine}`.c`
  (see :ref:`Target Description Macros and Functions <target-macros>`),
  possibly a target Makefile fragment :samp:`t- :samp:`{machine}``
...
* If necessary, a file :samp:`:samp:`{machine}` -modes.def` in the
  :samp:`:samp:`{machine}`` directory, containing additional machine modes to
  represent condition codes.  See :ref:`condition-code`, for further details.

* An optional :samp:`:samp:`{machine}`.opt` file in the :samp:`:samp:`{machine}``
...
The :samp:`:samp:`{machine}`.h` header is included very early in GCC's
standard sequence of header files, while :samp:`:samp:`{machine}` -protos.h`
is included late in the sequence.  Thus :samp:`:samp:`{machine}` -protos.h`
can include declarations referencing types that are not defined when
:samp:`:samp:`{machine}`.h` is included, specifically including those from
:samp:`rtl.h` and :samp:`tree.h`.  Since both RTL and tree types may not
be available in every context where :samp:`:samp:`{machine}` -protos.h` is
...
in :samp:`:samp:`{machine}`.h` rather than :samp:`:samp:`{machine}` -protos.h`.  
