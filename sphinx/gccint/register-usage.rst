...
.. index:: FIXED_REGISTERS

MacroFIXED_REGISTERS
...
.. index:: CALL_REALLY_USED_REGISTERS

MacroCALL_REALLY_USED_REGISTERS
.. index:: call-used register
...
.. function:: bool TARGET_HARD_REGNO_CALL_PART_CLOBBERED(unsigned intabi_id,unsigned intregno,machine_mode mode)
...
.. index:: INCOMING_REGNO

MacroINCOMING_REGNO(:samp:`{out}`)Define this macro if the target machine has register windows.  This C
...
.. index:: OUTGOING_REGNO

MacroOUTGOING_REGNO(:samp:`{in}`)Define this macro if the target machine has register windows.  This C
...
MacroLOCAL_REGNO(:samp:`{regno}`)Define this macro if the target machine has register windows.  This C
...
MacroPC_REGNUMIf the program counter has a register number, define this as that
...
.. index:: REG_ALLOC_ORDER

MacroREG_ALLOC_ORDERIf defined, an initializer for a vector of integers, containing the
...
.. index:: ADJUST_REG_ALLOC_ORDER

MacroADJUST_REG_ALLOC_ORDERA C statement (sans semicolon) to choose the order in which to allocate
...
.. index:: HONOR_REG_ALLOC_ORDER

MacroHONOR_REG_ALLOC_ORDERNormally, IRA tries to estimate the costs for saving a register in the
...
.. index:: IRA_HARD_REGNO_ADD_COST_MULTIPLIER

MacroIRA_HARD_REGNO_ADD_COST_MULTIPLIER(:samp:`{regno}`)In some case register allocation order is not enough for the
...
.. index:: HARD_REGNO_NREGS_WITH_PADDING

MacroHARD_REGNO_NREGS_WITH_PADDING(:samp:`{regno}`,:samp:`{mode}`)For values of :samp:`{regno}` and :samp:`{mode}` for which
...
MacroREGMODE_NATURAL_SIZE(:samp:`{mode}`)Define this macro if the natural size of registers that hold values
...
.. function:: bool TARGET_HARD_REGNO_MODE_OK(unsigned intregno,machine_mode mode)
...
.. index:: HARD_REGNO_RENAME_OK

MacroHARD_REGNO_RENAME_OK(:samp:`{from}`,:samp:`{to}`)A C expression that is nonzero if it is OK to rename a hard register
...
.. function:: bool TARGET_HARD_REGNO_SCRATCH_OK(unsigned intregno)
...
.. index:: AVOID_CCMODE_COPIES

MacroAVOID_CCMODE_COPIESDefine this macro if the compiler should avoid copies to/from ``CCmode``
...
.. index:: LEAF_REGISTERS

MacroLEAF_REGISTERSName of a char vector, indexed by hard register number, which
...
.. index:: LEAF_REG_REMAP

MacroLEAF_REG_REMAP(:samp:`{regno}`)A C expression whose value is the register number to which :samp:`{regno}`
...
MacroSTACK_REGSDefine this if the machine has any stack-like registers.

.. index:: STACK_REG_COVER_CLASS

MacroSTACK_REG_COVER_CLASSThis is a cover class containing the stack registers.  Define this if
the machine has any stack-like registers.

.. index:: FIRST_STACK_REG

MacroFIRST_STACK_REGThe number of the first stack-like register.  This one is the top
of the stack.

.. index:: LAST_STACK_REG

MacroLAST_STACK_REGThe number of the last stack-like register.  This one is the bottom of
the stack.
